...
class detector_class:
    ...
    def GMM_Inference(
        ...
        """Calculates the closest GMM to the given contrast\n"""
        ...
        for thickness_index in range(means.shape[0]):
            ...
            for k in range(len(contrast)):
                # calculate the distance for the current channel
                tmp = 0
                for h in range(k + 1):
                    tmp += (means[thickness_index, h] - contrast[h]) * inv_choleskys[
                        thickness_index, k, h
                    ]
    ...
    def infer_layer_from_contrast(self, contrast: np.ndarray):
        layer = detector_class.GMM_Inference(
            contrast,
            self.means,
            self.inv_cholesky_matrices,
    ...
    def create_contrast_image(self, image):
        ...
        return self.calc_contrast(image, mean_background_values)

    def create_flake_mask(self, contrast_image):
        return detector_class.mask_contrasted_image_ellipsoid_MULTI(
            contrast_image[:, :, self.used_channels],
